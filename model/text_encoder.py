...
class TextEncoder(BaseModule):
    def __init__(self, n_vocab, n_feats, n_channels, filter_channels, 
        ...
        self.encoder = Encoder(n_channels + spk_emb_dim * 2, filter_channels, n_heads, n_layers, 
                               kernel_size, p_dropout, window_size=window_size)

        self.proj_m = torch.nn.Conv1d(n_channels + spk_emb_dim * 2, n_feats, 1)
        self.proj_w = DurationPredictor(n_channels + spk_emb_dim * 2, filter_channels_dp, kernel_size, p_dropout)

    def forward(self, x, x_lengths, spk, emo):
