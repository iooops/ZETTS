...
if __name__ == "__main__":
    ...
    current_epoch = 0
    if params.checkpoint_step:
        model, optimizer, current_epoch = load_checkpoint(params.log_dir, model, optimizer, params.checkpoint_step)
        print("\n---Model Restored at Epoch {}---\n".format(current_epoch))
    ...
    for item in test_batch:
        mel, speaker = item['y'], item['speaker']
        i = int(speaker.cpu())
    ...
    iteration = 0
    for epoch in range(current_epoch + 1, n_epochs + 1):
        model.eval()
        print('Synthesis...')
        with torch.no_grad():
            for item in test_batch:
                ...
                spk = item['spk'].to(torch.float).unsqueeze(0)
                emo = item['emo'].to(torch.float).unsqueeze(0)
                speaker = item['speaker'].to(torch.long)
                i = int(speaker.cpu())
        ...
        torch.save({'model_state_dict': model.module.state_dict(), 'optimizer_state_dict': optimizer.state_dict()}, f=f"{log_dir}/grad_{epoch}.pt")
